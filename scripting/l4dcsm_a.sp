	...
	if (IsClientInGame(client)) 
	{
		if (GetClientTeam(client) == TEAM_SURVIVORS && g_bAnnounce)
		{
			switch (g_iSurvivorModels) 
			{
				...
				PrintToChat(client, "\x03[提示]\x01 输入 \x05!csm\x01 改变外观.");
				case CSM_SURVIVOR_CHARACTER_ONLY:
				PrintToChat(client, "\x03[提示]\x01 输入 \x05!csm\x01 改变角色.");
				case CSM_SURVIVOR_MODEL_AND_CHARACTER:
				PrintToChat(client, "\x03[提示]\x01 输入 \x05!csm\x01 改变角色或外观.");
			}
			
			switch (g_iInfectedModels) 
			{
				case CSM_INFECTED_MODEL_ONLY:
				PrintToChat(client, "\x03[提示]\x01 Type \x05!csm\x01 in chat to change your model to an infected (outside apperance).");
				case CSM_INFECTED_CHARACTER_ONLY:
				PrintToChat(client, "\x03[提示]\x01 Type \x05!csm\x01 in chat to change your character to an infected (outside appearance, abilities and sounds).");
				case CSM_INFECTED_CHARACTER_AND_MODEL:
				PrintToChat(client, "\x03[提示]\x01 Type \x05!csm\x01 in chat to change your character (outside appearance, abilities and sounds) or model (outside apperance) to an infected.");
			}
		}
	}
}
...
public Action:InitiateMenuAdmin(client, args) 
{
	if (client == 0) 
	{
		ReplyToCommand(client, "[提示] Character Select Menu is in-game only.");
		return;
	}
	if ((g_iChangeLimit == 0) || !g_bEnabled) 
	{
		ReplyToCommand(client, "[提示] Character Select Menu is currently disabled.");
		return;
	}
	if (g_iSurvivorModels == 0 && g_iInfectedModels == 0 && !g_bCommonModels)
	{
		ReplyToCommand(client, "[提示] Character Select Menu has no options available.");
		return;
	}
...
public Action:ShowMenu(client, args) 
{
	...
	{
		ReplyToCommand(client, "[提示] Character Select Menu is only available to admins.");
		return;
	}
	if (GetClientTeam(client) != TEAM_SURVIVORS)
	{
		ReplyToCommand(client, "[提示] Character Select Menu is only available to survivors.");
		return;
	}
	if (g_iSurvivorModels == 0 && g_iInfectedModels == 0 && !g_bCommonModels)
	{
	...
	{
		ReplyToCommand(client, "[提示] You must be alive to use the Character Select Menu!");
		return;
	}
	if (g_iClientChangeLimit[client] < 1) 
	{
		ReplyToCommand(client, "[提示] You cannot change your character again until you respawn.");
		return;
	}
	...
	if (g_bAnnounce)
	{
		if (g_iClientChangeLimit[param1] <= CSM_CHANGE_MESSAGE) 
		{
			if (g_iClientChangeLimit[param1] == 0)
				PrintToChat(param1, "\x03[提示]\x01 现在你的角色是 \x04%s\x01! 下次重生前你不能更改.", g_sCharacters[Survivor]);
			else if (g_iClientChangeLimit[param1] == 1)
				PrintToChat(param1, "\x03[提示]\x01 现在你的角色是 \x04%s\x01! 你只能改变 \x051\x01 次.", g_sCharacters[Survivor]);
			else
			PrintToChat(param1, "\x03[提示]\x01 现在你的角色是 \x04%s\x01! 你还能改变 \x05%i\x01 次.", g_sCharacters[Survivor], g_iClientChangeLimit[param1]);
		} 
		else 
		{
			PrintToChat(param1, "\x03[提示]\x01 现在你的角色是 \x04%s\x01!", g_sCharacters[Survivor]);
		}
	}
}
...
stock SetUpSurvivorModel(param1, Survivor) 
{
	...
	if (g_bAnnounce)
	{
		if (g_iClientChangeLimit[param1] <= CSM_CHANGE_MESSAGE)
			if (g_iClientChangeLimit[param1] == 0)
				PrintToChat(param1, "\x03[提示]\x01 现在你的角色是 \x04%s\x01! 下次重生前你不能更改.", g_sCharacters[Survivor+8]);
			else if (g_iClientChangeLimit[param1] == 1)
				PrintToChat(param1, "\x03[提示]\x01 现在你的角色是 \x04%s\x01! 你只能改变 \x051\x01 次.", g_sCharacters[Survivor+8]);
			else
		PrintToChat(param1, "\x03[提示]\x01 现在你的外观是 \x04%s\x01! 你还能改变 \x05%i\x01 次.", g_sCharacters[Survivor+8], g_iClientChangeLimit[param1]);
		else
		PrintToChat(param1, "\x03[提示]\x01 现在你的外观是 \x04%s\x01!", g_sCharacters[Survivor+8]);
	}
}
...
stock SetUpInfected(param1, Infected)
{
	...
	if (g_bAnnounce)
	{
		...
		{
			if (g_iClientChangeLimit[param1] == 0)
				PrintToChat(param1, "\x03[提示]\x01 现在你的角色是 \x04%s\x01! 下次重生前你不能更改.", g_sCharacters[Infected]);
			else if (g_iClientChangeLimit[param1] == 1)
				PrintToChat(param1, "\x03[提示]\x01 现在你的角色是 \x04%s\x01! 你只能改变 \x051\x01 次.", g_sCharacters[Infected]);
			else
			PrintToChat(param1, "\x03[提示]\x01 现在你的角色是 \x04%s\x01! 你还能改变 \x05%i\x01 次.", g_sCharacters[Infected], g_iClientChangeLimit[param1]);
		} 
		else 
		{
			PrintToChat(param1, "\x03[提示]\x01 现在你的角色是 \x04%s\x01!", g_sCharacters[Infected]);
		}
	}
...
stock SetUpInfectedModel(param1, Infected)
{
	...
	if (g_bAnnounce)
	{
		...
		{
			if (g_iClientChangeLimit[param1] == 0)
				PrintToChat(param1, "\x03[提示]\x01 现在你的角色是 \x04%s\x01! 下次重生前你不能更改.", g_sCharacters[Infected+8]);
			else if (g_iClientChangeLimit[param1] == 1)
				PrintToChat(param1, "\x03[提示]\x01 现在你的角色是 \x04%s\x01! 你只能改变 \x051\x01 次.", g_sCharacters[Infected+8]);
			else
			PrintToChat(param1, "\x03[提示]\x01 现在你的外观是 \x04%s\x01! 你还能改变 \x05%i\x01 次.", g_sCharacters[Infected+8], g_iClientChangeLimit[param1]);
		}
		else
		{
			PrintToChat(param1, "\x03[提示]\x01 现在你的外观是 \x04%s\x01!", g_sCharacters[Infected+8]);
		}
	}
}
...
stock SetUpCommonModel(param1, Infected)
{
	...
	if (g_bAnnounce)
	{
		if (g_iClientChangeLimit[param1] <= CSM_CHANGE_MESSAGE)
		{
			...
			PrintToChat(param1, "\x03[提示]\x01 现在你的外观是 \x04%s\x01! 你还能改变 \x05%i\x01 次.", g_sCharacters[Infected], g_iClientChangeLimit[param1]);
		}
		else
		{
			PrintToChat(param1, "\x03[提示]\x01 现在你的外观是 \x04%s\x01!", g_sCharacters[Infected]);
		}
	}
}
